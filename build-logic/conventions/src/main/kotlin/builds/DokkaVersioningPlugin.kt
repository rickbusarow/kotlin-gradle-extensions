...
abstract class DokkaVersionArchivePlugin : Plugin<Project> {
  override fun apply(target: Project) {
    ...
    fun Provider<Directory>.dir(subPath: String) = map { it.dir(subPath) }
    fun Provider<Directory>.file(subPath: String) = map { it.file(subPath) }

    val dokkaDir = target.layout.buildDirectory.dir("dokka")
    val dokkaHtmlMultiModuleBuildDir = dokkaDir.dir("htmlMultiModule")
    val currentVersionBuildDirZip = dokkaDir.file("$versionWithoutSnapshot.zip")

    val dokkaArchiveBuildDir = target.DOKKA_ARCHIVE_BUILD_DIR

    val dokkaArchive = target.DOKKA_ARCHIVE_DIR
    ...
    val unzip = target.tasks
      .register("unzipDokkaArchives", Sync::class.java) { task ->
        ...
        task.description =
          "Unzips all zip files in $dokkaArchive into ${dokkaArchiveBuildDir.get()}"
    ...
    val zipDokkaArchive = target.tasks
      .register("zipDokkaArchive", Zip::class.java) { task ->
        ...
        task.destinationDirectory.set(dokkaDir)
        task.archiveFileName.set(currentVersionBuildDirZip.map { it.asFile.name })
    ...
    target.tasks.register("syncDokkaToArchive", Copy::class.java) { task ->
      ...
      task.onlyIf {
        ...
        !destZip.exists() || !currentVersionBuildDirZip.get().asFile.zipContentEquals(destZip)
      }
    }
  ...
  companion object {
    val Project.DOKKA_ARCHIVE_DIR: File
      get() = rootProject.file("dokka-archive")
    val Project.DOKKA_ARCHIVE_BUILD_DIR: Provider<Directory>
      get() = rootProject.layout.buildDirectory.dir("tmp/dokka-archive")
  }
}
