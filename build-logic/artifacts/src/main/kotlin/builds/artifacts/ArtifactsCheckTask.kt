...
open class ArtifactsCheckTask @Inject constructor(
  ...
  private fun getExpectedArtifacts(): Map<String, ArtifactConfig> {
    ...
    val (ignoredArtifacts, testableArtifacts) =
      baselineArtifacts
  ...
  private fun StringBuilder.maybeAddMissingArtifactMessages(missing: List<ArtifactConfig>) =
    apply {
  ...
  private fun pluralsString(size: Int): String = if (size == 1) {
    "This artifact is"
  } else {
    "These artifacts are"
  }
